...
function CartPopoverItem(props: { item: productForCart }) {
  const { item } = props;
  return (
    <Card sx={{ display: 'flex', width: '600px' }} key={item.product.id}>
      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
        <CardContent sx={{ flex: '1 0 auto' }}>
          ...
          <Stack alignItems={'center'} direction={'row'}>
            <Typography component='div' variant='subtitle2'>Color:</Typography><ColorCircle color={item.color}/>
          </Stack>
      ...
      <Link to={`/item/${item.product.id}`}><CardMedia
        component='img'
        sx={{ width: 151 }}
